...
export default function ServiceSummaryTelevision({ route }) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const documentSnapshot = await getDoc(doc(db, 'television', docId));
	...
	const handleDeleteConfirm = async () => {
		try {
			await deleteDoc(doc(db, 'television', docId));
